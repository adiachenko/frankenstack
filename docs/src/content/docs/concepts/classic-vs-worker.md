...
In <a href="https://frankenphp.dev/docs/classic/" target="_blank">classic</a> mode, PHP boots fresh for each request. This is the traditional PHP execution model.
...
- Trade-off: Slower performance due to per-request bootstrap, higher incremental memory cost per request.

In <a href="https://frankenphp.dev/docs/worker/" target="_blank">worker</a> mode, FrankenPHP keeps PHP workers alive between requests, eliminating bootstrap overhead.
